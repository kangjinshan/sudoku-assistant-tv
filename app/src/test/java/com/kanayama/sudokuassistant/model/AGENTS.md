...
`solverRejectsUnsolvablePartialBoardWithoutChangingGivens` 验证候选数最少优先求解器的失败分支与题面不可变性；全部生成题测试同时断言成功求解不改变题面，包含旧按行回溯慢样本（九宫专家 seed 75）。
