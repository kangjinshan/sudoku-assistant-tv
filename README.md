...
四个初始数字都必须使用，最后只剩下 24 即为成功。减法允许得到负数；除法必须整除。计算走不通时可选择“重置”恢复当前题目的四个初始数字，也可选择“换一题”。
...
- 24 点：方向键在数字、运算符与操作按钮间移动，确定键依次选择；菜单键快速重置原题，返回键回到首页
...
测试覆盖四宫、六宫、九宫的全部难度，以及 24 点题目的数字范围、整数可解性、整除约束、数字合并和重置行为。
